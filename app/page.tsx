...
const timerSeconds: Record<string, number> = { "warmup-walk": 300, "row-easy": 600, "row-intervals": 720, "treadmill-intervals": 1440, "treadmill-cooldown": 300, cooldown: 300 };
...
export default function Home() {
  ...
  if (activeExercise && activeDay && progress.activeWorkout) {
    ...
    return <main className="app-shell exercise-screen">
      ...
      {activeExercise.treadmillPlan && <section className="card treadmill-settings"><div className="treadmill-settings-heading"><div><p className="eyebrow">TREADMILL SETTINGS</p><h2>Use feel first, speed second</h2></div><span>Start low</span></div><div className="treadmill-stage-list">{activeExercise.treadmillPlan.map((stage) => <article key={`${stage.time}-${stage.speed}`}><strong>{stage.time}</strong><div><b>{stage.speed}</b><small>Speed</small></div><div><b>{stage.incline}</b><small>Incline</small></div><p>{stage.effort}</p></article>)}</div><p className="treadmill-adjustment">Begin at the lowest listed speed. Adjust by only 0.1–0.2 mph until the effort description feels right. Slower is always okay.</p></section>}
