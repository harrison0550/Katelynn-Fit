...
export default function Home() {
  ...
  if (activeExercise && activeDay && progress.activeWorkout) {
    ...
    return <main className="app-shell exercise-screen">
      ...
      {activeExercise.media ? <button className="exercise-hero media-hero" onClick={() => setExpandedMedia(activeExercise.media)} aria-label={`Enlarge ${activeExercise.name} demonstration`}><img src={`${MEDIA_BASE}${activeExercise.media.src}`} alt={activeExercise.media.alt} /><span className="media-badge">Tap to enlarge</span></button> : <div className="exercise-hero"><span>{activeExercise.icon}</span></div>}
  ...
  return <div className="app-shell"><header className="topbar"><div><p className="brand-kicker">KATELYNN&apos;S HOME GYM</p><h1>Katelynn Fit</h1></div><div className="avatar" aria-hidden="true">KF</div></header>{notice && <div className="notice" role="status">{notice}</div>}<main>
    ...
    {tab === "plan" && <section><p className="eyebrow">WEEKLY PLAN</p><h2 className="page-title">Five days to feel stronger</h2><p className="page-intro">Three strength days and two cardio discovery days. Weekends are for recovery or an easy family activity.</p><div className="plan-list">{PROGRAM.map((day) => <article className="card plan-card" key={day.id}><div className="plan-icon">{day.icon}</div><div><p className="eyebrow">{day.day} · {day.duration}</p><h3>{day.title}</h3><p>{day.focus}</p><button className="text-button" onClick={() => startWorkout(day)}>{progress.completed[day.id] ? "Repeat workout" : "View workout"} →</button></div></article>)}</div></section>}
